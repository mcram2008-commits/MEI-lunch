...
export default function AdvisorPortal() {
    ...
    const [activeTab, setActiveTab] = useState<"pending" | "profile" | "history">("pending");
    ...
    useEffect(() => {
        ...
        const update = () => {
            ...
            const history = allPasses.filter(p => {
                ...
                if (!student) return false;
                const studentClass = `${student.department}-${student.year}-${student.section}`.toUpperCase();
                return p.type === "leave" && (p.status === "approved" || p.status === "rejected") && studentClass === user.assignedClass.toUpperCase();
            ...
            <header className="fixed top-0 bg-gradient-to-r from-[#1e3a8a] to-[#4338ca] text-white w-full h-16 flex items-center px-6 z-50">
                ...
                <div className="ml-6">
                    <h1 className="font-bold text-lg tracking-tight capitalize">
                        {activeTab === 'pending' ? 'Advisor Approval' : activeTab === 'history' ? 'Leave History' : 'Profile'}
                    </h1>
                </div>
            </header>
                ...
                {activeTab === "history" && (
                    <div className="animate-in fade-in space-y-6">
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <h2 className="font-black text-gray-500 uppercase tracking-widest text-xs">Past Decisions (Leave Only)</h2>
                        </div>

                        {historyRequests.length === 0 ? (
                            <div className="text-center py-24 bg-white rounded-3xl border border-gray-100 italic font-bold text-gray-300">No leave history available</div>
                        ) : (
                            historyRequests.map(pass => {
                                ...
                                return (
                                    <div key={pass.id} className="bg-white rounded-2xl shadow-sm border p-6 space-y-4 animate-in slide-in-from-bottom-4 relative overflow-hidden">
                                        <div className={`absolute top-0 right-0 px-4 py-1 text-[8px] font-black uppercase tracking-widest ${pass.status === 'approved' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                            {pass.status}
                                        </div>
                                        <div className="flex items-center gap-4 border-b pb-4">
